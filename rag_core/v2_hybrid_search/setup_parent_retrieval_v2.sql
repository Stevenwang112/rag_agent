...
$$;
-- 6. Create Materialized View for Company Mapping (OPTIMIZATION)
-- This creates a fast lookup table: parent_id -> company_name
-- It parses the 'source' or any other logic to determine company mapping once.
-- Ideally, the backfill script put 'company_name' on chunks, so simple filter works.
-- BUT, if we want to filter chunks based on PARENT properties efficiently without relying on chunk duplication:

-- For now, relying on the 'company_name' metadata on CHUNKS is the fastest path as enabled by previous step.
-- The query in `match_parent_chunks_hybrid` already does: `where dc.metadata @> filter`
-- Since we updated chunk metadata, this is efficient (esp with JSONB GIN index, which we should add).

-- 6. Add JSONB Index on metadata for fast filtering
create index if not exists idx_document_chunks_metadata on document_chunks using gin (metadata);

-- The function `match_parent_chunks_hybrid` is already correct:
-- `where dc.metadata @> filter` will use the GIN index on metadata to quickly find chunks with {"company_name": "ES9"}.

-- No further SQL changes needed if function already has `filter` param and WHERE clause.
